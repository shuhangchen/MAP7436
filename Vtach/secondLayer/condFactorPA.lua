...
function condFactorPA.train(opt,j,k,nt,nc)
   ...
   hexperName = '/home/mark/Documents/VT_learning/Logs/factor2layer/MES_j'..tostring(j)..'k'..tostring(k)..'nt'..tostring(nt)..'nc'..tostring(nc)
   ...
   -- load the QRS data
   qrsData_loaded = torch.load('/home/mark/Documents/VT_learning/squarePA/firstLayer/candidates/qrsLayer1.t7')
   ...
   while epoch < opt.maxNumEpoch do
	 ...
	 local feval = function(x)
	    ...
	    local function VTPACriterion(input, VTPAlabels)
	       ...
	       local errSquare = torch.cmul(distanceMain, (torch.add(VTPAlabels[{{},{1}}],1):mul(0.5)))
	       local errHinge = 0.5*opt.hingeCoef*errSquare:mean()
		-- backpropagate
	       local gradHingeMain = torch.Tensor(distanceMain:size()):fill(1/opt.batchSize)
	       if opt.cuda then
		  gradHingeMain = gradHingeMain:cuda()
	       end
	       ...
	       gradHinge[1] =  torch.mul(gradOutputMain[1], 0.5*opt.hingeCoef)
	       gradHinge[2] =  torch.mul(gradOutputMain[2], 0.5*opt.hingeCoef)
   ...
   if opt.trainVT then
      torch.save(hexperName..'/cond_VT.net', factor)
      torch.save(hexperName..'/cond_lossFun_vt.t7', lossFunTrack)
   else
      torch.save(hexperName..'/cond_PA.net', factor)
      torch.save(hexperName..'/cond_lossFun_PA.t7', lossFunTrack)
   end
